...
import type { Domain, Slide } from '@/types';
...
const DOMAIN_ORDER: Domain[] = ['strategy', 'management', 'technology'];

const DOMAIN_HEADING_COLOR: Record<Domain, string> = {
  strategy: 'text-blue-700 border-blue-200 bg-blue-50',
  management: 'text-green-700 border-green-200 bg-green-50',
  technology: 'text-purple-700 border-purple-200 bg-purple-50',
};
...
export const SlidesPage = () => {
  ...
  const getSlideMastered = (slide: Slide) => {
      ...
      {filter === 'all' ? (
        ...
        <div className="space-y-6">
          {DOMAIN_ORDER.map((domain) => {
            ...
            return (
              <div key={domain}>
                <div className={cn('flex items-center gap-2 px-3 py-2 rounded-lg border mb-3', DOMAIN_HEADING_COLOR[domain])}>
                  <span className="text-sm font-bold">{domainLabel[domain]}</span>
                  <span className="text-xs opacity-70">{domainSlides.length}スライド</span>
                </div>
                <div className="space-y-3">
                  {domainSlides.map(renderSlideCard)}
                </div>
              </div>
            );
          })}
        ...
        /* Filtered flat list */
        <div className="space-y-3">
          {slides.filter((s) => s.domain === filter).map(renderSlideCard)}
        </div>
      )}
    </div>
  );
